...
public class BullyBot extends ChickenBot {
  ...
  public static final char BULLY_BOT_PERSONALITY = 'B';
  /**
  ...
  public BullyBot( int id, int jolts ) {
    super( id, jolts );
    personality = BULLY_BOT_PERSONALITY;
  }
  ...
  public int [] fireCannon() {
    ...
    int desiredEnergy = energyLevel - 2;
    if( count > desiredEnergy ) {
      count = desiredEnergy;
    }
    ...
    if( count > 0 ) {
      ...
      for(int i = District.NORTH; i < botsSensed.length; i++ ) {
        // something not right with this expression
